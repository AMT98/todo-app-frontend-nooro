...
const TaskCard: React.FC<TaskCardProps> = ({
  ...
  return (
    ...
    <div className="flex space-x-2">
      ...
      <Link
      href={`/update-task/${id}`}>
        <h3 className={`max-w-[632px] text-sm font-semibold  text-[#F2F2F2]  ${isComplete && "line-through opacity-60 text-[#808080]"}`}>{title}</h3>
        {/* <p className={`text-sm text-${color}-500`}>Color: {color}</p> */}
      </Link>
